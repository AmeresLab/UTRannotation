...
countingWindowOverlap_plus = read.table("/Volumes//groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/refSeq_mrna_utrsPresent_plus_complement_complement_countingWindows.bed")
countingWindowOverlap_minus = read.table("/Volumes//groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/refSeq_mrna_utrsPresent_minus_complement_complement_countingWindows.bed")

## overlap can happen on both strands, so remove + from - and vise versa
countingWindowOverlap_plus = countingWindowOverlap_plus %>% filter(V6 == "+")
countingWindowOverlap_minus = countingWindowOverlap_minus %>% filter(V6 == "-")

#### connecting trasncript to fragment - reading in the intersection between  flattened 'fragments'and UTR anntoation:



transcriptToFragment_minus= read.table("/Volumes//groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/refSeq_mrna_utrsPresent_minus_intersect.bed",sep="\t",stringsAsFactors = F)
transcriptToFragment_plus= read.table("/Volumes//groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/refSeq_mrna_utrsPresent_plus_intersect.bed",sep="\t",stringsAsFactors = F)
...
countingWindowOverlap_minus = countingWindowOverlap_minus[!duplicated(countingWindowOverlap_minus),]
...
for(i in 1:nrow(countingWindowOverlap_minus)){
  ...
  if(length(unique(doesTranscriptExistInOtherFragment$V4))==1){
    ...
    transcriptsInRefSeq = refSeqAnnotation[is.element(refSeqAnnotation$V7, doesTranscriptExistInOtherFragment$V11),]
...
countingWindowOverlap_plus = countingWindowOverlap_plus[!duplicated(countingWindowOverlap_plus),]
for(i in 1:nrow(countingWindowOverlap_plus)){
  ...
  if(length(unique(doesTranscriptExistInOtherFragment$V4))==1){
    ...
    countingWindowOverlap_plus[i,]$transcriptName = transcriptsInRefSeq$V7
...
write.table(inaccurateAnnotationStarts,"/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/inaccurateAnnotationStarts.bed",sep="\t",row.names = F,col.names = F,quote = F)

inaccurateAnnotationStarts_rearranged = cbind.data.frame(inaccurateAnnotationStarts[,c("V1","V2","V3","V4","V5","V6","utrStr_chr","utrStr_start","utrStr_end")])
write.table(countingWindowOverlap_plus,"/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/test_plus.bed",sep="\t",row.names = F,col.names = F,quote = F)



write.table(countingWindowOverlap_minus,"/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/test_monus.bed",sep="\t",row.names = F,col.names = F,quote = F)

write.table(inaccurateAnnotationStarts_rearranged,"/Users/pooja.bhat/Dropbox/UTRannotation/mESC/currentAnnotation_withThomas/annotationStarts//inaccurateAnnotationStarts.bed",sep="\t",row.names = F,col.names = F,quote = F)
...
write.table(inaccurateAnnotationStarts_rearranged,"/Users/pooja.bhat/Dropbox/UTRannotation/mESC/currentAnnotation_withThomas/annotationStarts//inaccurateAnnotationStarts_igv.bed",sep="\t",row.names = F,col.names = F,quote = F)
...
refSeqAnnotation = read.table("/Volumes/groups/bioinfo/shared/Ameres.GRP/incoming/3PrimeEndAnnotation_OLD/annotations_mm10/refSeq_mm10_GRCm38_06-march-2017/processed/refSeq_mrna_utrsPresent.bed",stringsAsFactors = F)
overlappingCountingWindows_plus = read.table("/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/overlappingCountingWindows_refSeq_plus.bed",stringsAsFactors = F)
...
overlappingCountingWindows_minus = read.table("/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/overlappingCountingWindows_refSeq_minus.bed",stringsAsFactors = F)
...
for(i in 1:length(overlappingCountingWindows_minus_split)){
  a = which.max((overlappingCountingWindows_plus_split[[i]]$V18))
...
write.table(overlappingCountingWindows_rearranged,"/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/overlappingCountinngWindows_final.bed",sep="\t",quote = F,row.names = F)

write.table(overlappingCountingWindows_rearranged,"/Users/pooja.bhat/Dropbox/UTRannotation/mESC/currentAnnotation_withThomas/annotationStarts/overlappingCountinngWindows_final.bed",sep="\t",quote = F,row.names = F)

inaccurateAnnotationStarts = inaccurateAnnotationStarts[,c("V1","V2","V3","V4","V5","V6","utrStr_chr","utrStr_start","utrStr_end","transcriptName")]
totalRefSeq = rbind(overlappingCountingWindows_rearranged,inaccurateAnnotationStarts)
totalRefSeq = totalRefSeq[!duplicated(totalRefSeq),]
write.table(totalRefSeq,"/Volumes/groups/ameres/Pooja/Projects/wholePipeline/pipeline_1/createStarts/finalStarts/starts_refSeqOverlappers.bed",sep="\t",quote = F,col.names = F,row.names = F)
